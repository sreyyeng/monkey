...
// @version      3.6
// @description  【兼容增强】支持普通详情页与短剧播放页，增强剧集列表识别和动态刷新
...
(function() {
    ...
    function createPanel() {
        ...
        panel.innerHTML = `
            ...
            <div id="iyf-helper-header">iyf M3U8 助手 v3.6</div>
    ...
    function setupM3U8Interception() {
        ...
        function handleM3U8Found(m3u8Url, capturedNonce, capturedEpisodeId) {
            ...
            const episode = episodes.find(ep => ep.id === capturedEpisodeId);
    ...
    function updateCommandsDisplay() {
        const capturedCommands = episodes.filter(ep => ep.m3u8).map(ep => `"${ep.m3u8}" --saveName "${videoTitle} ${ep.title}" --enableDelAfterDone`);
    ...
    function getEpisodeIdFromHref(href) {
        if (!href) return null;
        try {
    ...
    function updateActiveEpisodeIndicator() {
        ...
        episodes.forEach(ep => {
            if (ep.element) {
                ep.element.classList.toggle('active', Boolean(currentId && ep.id === currentId));
            }
        });
    }
    ...
    function initialize() {
        ...
        // 同时兼容 /detail/影片ID 和 /play/影片ID 两种地址。
        const pathParts = window.location.pathname.split('/').filter(Boolean);
        const pageType = pathParts[0] || '';
        const newVideoId = pathParts[1] || '';
        if (!newVideoId || !['detail', 'play'].includes(pageType)) {
            console.log('[iyf助手] 当前不是可识别的详情/播放页面');
            return false;
        }
        ...
        // 模式A：原普通详情页；模式B：新增短剧播放页。
        const pageModes = [
            {
                name: '普通剧集',
                titleSelector: 'h4.d-inline.h4',
                listSelector: 'div.n-media-list',
                episodeSelector: 'a.media-button',
                reverse: true
            },
            {
                name: '短剧',
                titleSelector: '.short-drama-title',
                listSelector: '.short-drama-episode-list',
                episodeSelector: 'a.media-button',
                reverse: false
            }
        ];
        ...
        for (const mode of pageModes) {
            const title = document.querySelector(mode.titleSelector);
            const container = document.querySelector(mode.listSelector);
            const links = container ? Array.from(container.querySelectorAll(mode.episodeSelector)) : [];
            if (title && container && links.length > 0) {
                matchedMode = mode;
                titleElement = title;
                episodeListContainer = container;
                episodeLinks = links;
                break;
            }
        }

        if (!matchedMode || !titleElement || !episodeListContainer || episodeLinks.length === 0) {
            console.log('[iyf助手] 页面元素未就绪，尚未识别到普通剧集或短剧列表');
            return false;
        }
        ...
        const normalizedLinks = matchedMode.reverse ? [...episodeLinks].reverse() : episodeLinks;
        const newSignature = normalizedLinks
            .map(link => `${link.getAttribute('href') || ''}|${link.getAttribute('title') || link.innerText}`)
            .join('||');
        const oldSignature = episodes.map(ep => `${ep.href || ''}|${ep.title}`).join('||');

        // 不只比较集数，避免网站动态换剧但集数相同而无法刷新。
        if (episodes.length === 0 || newSignature !== oldSignature) {
            videoTitle = titleElement.textContent.trim();
            const titleEl = document.getElementById('iyf-helper-title');
            if (titleEl) titleEl.innerText = `影片: ${videoTitle}`;

            const oldM3U8ById = new Map(episodes.filter(ep => ep.id && ep.m3u8).map(ep => [ep.id, ep.m3u8]));
            episodes = normalizedLinks.map(link => {
                const href = link.getAttribute('href') || '';
                const id = getEpisodeIdFromHref(href);
                return {
                    id,
                    title: link.getAttribute('title')?.trim() || link.textContent.trim(),
                    href,
                    originalLink: link,
                    m3u8: id ? (oldM3U8ById.get(id) || null) : null,
                    element: null
                };
            });

            renderEpisodeList();
            console.log(`[iyf助手] ${matchedMode.name}模式初始化成功，找到 ${episodes.length} 集`);
        } else {
            // Angular重绘后DOM节点会变化，即使剧集内容未变也要更新点击目标。
            episodes.forEach((episode, index) => {
                episode.originalLink = normalizedLinks[index] || episode.originalLink;
            });
        }
        ...
        episodes.forEach(episode => {
            ...
            item.addEventListener('click', () => {
                if (episode.originalLink && !item.classList.contains('active')) {
                    requestNonce++;
                    pendingEpisodeId = episode.id || getEpisodeIdFromHref(episode.href);
